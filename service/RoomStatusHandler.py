...
class RoomStatusHandler:
    ...
    async def _build_room_status_by_phase(self, room: GameRooms, base_info: Dict[str, Any], user_id: int) -> Dict[str, Any]:
        ...
        status_data = {
            "room": base_info,
            "script": self._build_script_info(room.script) if room.script else None
        }
        ...
        elif room.status == "进行中" or room.status == "搜证中":
            # 进行中阶段：返回完整的游戏信息
            status_data["players"] = await self._build_players_info(room)
            # print("123123123")
    ...
    def _build_script_info(self, script) -> Dict[str, Any]:
        """构建剧本基础信息"""
        if not script:
            return None
